...
def env_handler(request, *callback_args, **callback_kwargs):
    ...
    context = RequestContext(request, {
        ...
        'brand': brand,
        'timestamp': _format_time(datetime.now()),
...
def brand_handler(request, *callback_args, **callback_kwargs):
    ...
    brand = callback_kwargs['brand']
    bigscreen = request.GET.get('bigscreen')
    refresh_seconds = 60
    timestamp_now = datetime.now()
    timestamp_next = _get_next_refresh_time(timestamp_now, refresh_seconds)
    ...
    context = RequestContext(request, {
        ...
        'timestamp': _format_time(timestamp_now),
        'refresh_seconds': refresh_seconds,
        'next_refresh_timestamp': _format_time(timestamp_next),
        'icon_class': _get_icon_css_class(bigscreen),
        'bigscreen': bigscreen
    })
...
def _get_next_refresh_time(timestamp_now, refresh_seconds):
    return timestamp_now + timedelta(seconds=refresh_seconds)


def _format_time(timestamp):
    return portlet_services.format_time(timestamp)


def _get_icon_css_class(bigscreen):
    if bigscreen == "true":
        return "icon_medium"
    else:
        return "icon"
